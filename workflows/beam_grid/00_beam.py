...
mat = ElasticIsotropic(E=E, v=v, density=density)
sec = ISection(w=w, h=h, tw=tw, tf=tf, material=mat)

lines = [
    Line(
        Point(c * length.to_base_units().magnitude / n, 0, 0),
        Point((c + 1) * length.to_base_units().magnitude / n, 0, 0),
    )
    for c in range(n)
]
...
B = mdl.find_nodes_around_point(
    Point(length.to_base_units().magnitude, 0, 0), distance=1
)[0]
...
stp.add_node_pattern(nodes=B, y=-1 * units.kN, load_case="LL")
...
prb.show_displacements_contour(show_bcs=0.5)
...
print(disp.get_min_result(2, step=stp).vector)
