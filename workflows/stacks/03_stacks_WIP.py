...
from compas_fea2.model import (Model, DeformablePart, BeamElement, Node, ShellElement,
...
compas_mesh: Mesh = model.mesh_to_compas()

levels = [compas_mesh.translated(Vector(0, 0, n*H_STACK.to_base_units().magnitude)) for n in range(N_STACK+1)]
...
for level in levels:
    vertex_node = {vertex: prt.add_node(Node(level.vertex_coordinates(vertex))) for vertex in level.vertices()}

    for face in level.faces():
        nodes = [vertex_node[vertex] for vertex in level.face_vertices(face)]
        element = ShellElement(nodes=nodes, section=shell_sec, implementation='shelldkgt')
        prt.add_element(element)
mdl.add_part(prt)

# # Add deformable part from GMSH model
# levels_prt = mdl.add_parts([DeformablePart.shell_from_compas_mesh(level, section=shell_sec, implementation='shelldkgt') for level in levels])
