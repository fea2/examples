...
lines_bottom = [Line(Point(c*length/n, 0, 0), Point((c+1)*length/n, 0, 0)) for c in range(n)]
lines_top = [Line(Point(c*length/n, 0, 1000), Point((c+1)*length/n, 0, 1000)) for c in range(n)]
prt= DeformablePart.from_compas_lines(lines_top, section=sec)
prt2= DeformablePart.from_compas_lines(lines_bottom, section=sec)
mdl.add_parts([prt, prt2])
